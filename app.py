...
Fluxo:
1. Upload arquivo
2. Auto-detectar modelo (COM CONFIANÇA)
3. Confirmar ou trocar modelo
4. Pipeline: load → normalize → validate → render
5. Exportar
"""
...
<div style="text-align:center;padding:1.2rem 0 .8rem">
  ...
  <p style="color:#94a3b8;font-size:.7rem;margin:.1rem 0 0">
    Auto-detect · Multi-modelos · v1.0
  </p>
</div>
""", unsafe_allow_html=True)
...
uploaded = st.sidebar.file_uploader(
    "Escolha arquivo",
    type=["xlsx", "xls", "csv"],
    help="Planilha (Excel/CSV) com dados para análise"
)
...
if uploaded is None:
    ...
    <div class="page-header">
      ...
      <p>Plataforma de análise multi-modelos com detecção automática</p>
    </div>
    """, unsafe_allow_html=True)
    
    col1, col2, col3 = st.columns(3)
    col1.info("**💬 Inteligente**\n\nDetecta modelo automaticamente")
    col2.info("**🚀 Rápido**\n\nNormalizações automáticas")
    col3.info("**📊 Completo**\n\nKPIs, gráficos, export")
...
st.markdown(f"""
<div class="page-header">
  <h1>{model.MODEL_ICON} {model.MODEL_NAME}</h1>
  <p>{uploaded.name} • {len(model.df):,} linhas • {ts_now}</p>
</div>
""", unsafe_allow_html=True)

# Filtros
st.markdown('<div class="filter-box"><b>🔍 Filtros</b></div>', unsafe_allow_html=True)

filter_cols = [c for c in model.FILTER_COLS if c in model.df.columns]
if not filter_cols:
    filter_cols = [c for c in model.df.columns if model.df[c].dtype == 'object'][:3]

filtered = model.df.copy()

if filter_cols:
    cols_filter = st.columns(min(4, len(filter_cols)))
    for idx, col_name in enumerate(filter_cols):
        with cols_filter[idx % 4]:
            options = sorted(model.df[col_name].dropna().astype(str).unique().tolist())
            if len(options) <= 50:
                sel = st.multiselect(col_name, options, default=options, key=f"f_{col_name}")
                if sel:
                    filtered = filtered[filtered[col_name].astype(str).isin(sel)]

if filtered.empty:
    st.warning("⚠️ Nenhum dado com estes filtros")
    st.stop()
...
for tab, tab_name in zip(tabs, model.TABS):
    with tab:
        st.info(f"✅ {tab_name} | {len(filtered):,} linhas")
        
        if st.checkbox("Mostrar dados", key=f"show_{tab_name}"):
            st.dataframe(filtered.head(20), use_container_width=True)

st.markdown("---")
st.caption("📊 Power BI Automático © 2026")
import io
import re
import warnings
from datetime import datetime
