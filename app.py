...
# -*- coding: utf-8 -*-
"""
SCIENTIFIC PRD · Sistema de Comissões v6.0
Dashboard analítico com dados prontos da planilha

Nova premissa:
- A planilha já vem com os valores calculados.
- O Python NÃO recalcula comissão, percentual ou base.
- O app apenas lê, normaliza, filtra, agrega e exibe.
- As únicas contas feitas no app são somas, contagens e médias simples
  para dashboards e tabelas analíticas.
"""
...
# ══════════════════════════════════════════════════════════
# COLUMN MAP — canonical → aliases esperados na planilha pronta
# ══════════════════════════════════════════════════════════
COL_MAP = {
...
def load_file(f):
    ...
    if name.endswith((".xlsx", ".xls")):
        for hrow in [0, 1, 2]:
            try:
                df = pd.read_excel(io.BytesIO(content), dtype=str, header=hrow, na_filter=False)
...
def normalize_columns(df: pd.DataFrame) -> pd.DataFrame:
    ...
    text_cols = [
        "BranchName", "ReceiptNumber", "EventType", "InvDocNum", "ObjectType",
        "CardCode", "CardName", "SlpCode", "SlpName", "ItemCode", "ItemDescription",
        "Installment", "Currency", "GLAccount", "GLAccountName",
    ]
    for c in text_cols:
        if c in df.columns:
...
with st.sidebar:
    ...
    show_zero_commission = st.checkbox(
        "Incluir linhas sem comissão",
        value=True,
        help="Quando desmarcado, exibe apenas linhas com comissão final maior que zero."
    )
    show_zero_received = st.checkbox(
        "Incluir linhas sem recebimento",
        value=True,
        help="Quando desmarcado, exibe apenas linhas com valor recebido maior que zero."
    )
    st.markdown("---")
...
if uploaded is None:
    ...
    st.markdown("""
    - lê a planilha e identifica as colunas principais;
...
st.markdown("""
<div class="note-box">
<b>Nova lógica:</b> esta versão não recalcula comissão. Ela usa os campos prontos da planilha, agrupando e exibindo os dados de forma analítica.
</div>
""", unsafe_allow_html=True)
...
if has_dates and date_range and len(date_range) == 2:
    filtered = filtered[filtered["ReceiveDate"].between(pd.Timestamp(date_range[0]), pd.Timestamp(date_range[1]))]
...
with t_dash:
    ...
    fig_v.update_layout(showlegend=False, coloraxis_showscale=False, height=420, plot_bgcolor="white", paper_bgcolor="white")
    ...
    fig_items.update_layout(showlegend=False, coloraxis_showscale=False, height=400, plot_bgcolor="white", paper_bgcolor="white")
...
with t_filial:
    ...
    fig_f = px.bar(
        ...
        color="Comissao",
        color_continuous_scale=["#c4b5fd", "#4f46e5"],
        labels={"Comissao": "R$", "Filial": ""},
    )
    fig_f.update_traces(textposition="outside")
    fig_f.update_layout(showlegend=False, coloraxis_showscale=False, height=max(320, len(chart_f) * 60), plot_bgcolor="white", paper_bgcolor="white")
...
with t_cli:
    ...
    fig_cli.update_layout(showlegend=False, coloraxis_showscale=False, height=420, plot_bgcolor="white", paper_bgcolor="white")
...
with t_det:
    show_cols = [c for c in [
        "ReceiveDate", "ReceiveMonth", "BranchName", "ReceiptNumber", "EventType", "InvDocNum",
        "CardCode", "CardName", "SlpCode", "SlpName", "ItemCode", "ItemDescription",
        "Installment", "Quantity", "Currency", "LineTotal", "AmountReceived",
        "AllocatedExpenses", "PaymentBase", "CommissionPct", "CommissionValue",
    ...
    if search_all:
        mask = detail.astype(str).apply(lambda col: col.str.contains(search_all, case=False, na=False)).any(axis=1)
        detail = detail[mask]
    if vend_d != "Todos":
